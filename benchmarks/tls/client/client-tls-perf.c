 ...
 */
static int CreateSocketConnect(int port, socklen_t* socketfd, char* ip)
{
    int                on = 1;
...
static void Usage(void)
{
    ...
    printf("-i <ip>     The IP of the server\n");
}
...
int main(int argc, char* argv[])
{
    ...
    for (i = 0; !SSLConn_Done(sslConnCtx); i = (i + 1) % numConns) {
        ...
        if ((sslConn->state == INIT) &&
            ...
            if (CreateSocketConnect(port, &socketfd, ip) == EXIT_FAILURE) {
