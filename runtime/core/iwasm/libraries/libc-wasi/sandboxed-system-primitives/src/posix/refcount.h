...
inline void refcount_init(struct refcount *r, unsigned int count) PRODUCES(*r) {
  atomic_init(&r->count, count);
}

// Increment the reference counter.
inline void refcount_acquire(struct refcount *r) PRODUCES(*r) {
  atomic_fetch_add_explicit(&r->count, 1, memory_order_acquire);
}

// Decrement the reference counter, returning whether the reference
// dropped to zero.
inline bool refcount_release(struct refcount *r) CONSUMES(*r) {
